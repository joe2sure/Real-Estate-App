...
class AuthProvider with ChangeNotifier {
  ...
  bool get isLoggedIn => _currentUser != null && _token != null;
    ...
    //clearing auth without API call
  Future<void> clearAuth() async {
    try {
      ...
      _errorMessage = null;
      notifyListeners();
      debugPrint('Auth cleared successfully');
    } catch (e) {
  ...
  bool isTokenValid() {
    ...
    try {
      ...
      return DateTime.now().add(const Duration(minutes: 5)).isBefore(expiryDate);
    } catch (e) {
  ...
  Future<void> init() async {
    ...
    if (userJson != null && token != null) {
      ...
      if (!isTokenValid()) {
        debugPrint('Stored token is expired, clearing auth');
        await clearAuth();
      } else {
        notifyListeners();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          ...
          final box = await Hive.openBox('auth');
          await box.put('user', jsonEncode(data['data']));
          notifyListeners();
        }
      }
    } catch (e) {
      // Silently fail - user data will remain as cached
