...
class PaymentProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>> recordPayment(
    ...
    try {
      ...
      // Call the updated service method that returns full response
      final result = await PaymentService.recordPayment(token, paymentData);
      
      // Extract the payment object
      final newPayment = result['payment'] as Payment;
      ...
      debugPrint('📧 Invoice number: ${result['invoiceNumber']}');
      debugPrint('📧 Invoice email sent: ${result['invoiceEmailSent']}');
      
      if (result['invoiceEmailError'] != null) {
        debugPrint('⚠️ Invoice email error: ${result['invoiceEmailError']}');
      }
      
      // Return the complete result including invoice details
      return {
        'success': true,
        'payment': newPayment,
